...
# Output
set output "pdf/plot_goodput_total_data_sent_vs_runtime.pdf"
...
# Axes labels
set xlabel "Traffic goodput total data sent (Gbit)" # Markup: e.g. 99^{th}, {/Symbol s}, {/Helvetica-Italic P}
set ylabel "Runtime (s)"

# Axes ranges
set xrange [0.1:]       # Explicitly set the x-range [lower:upper]
set yrange [1:]       # Explicitly set the y-range [lower:upper]
...
plot    "../../ns3_experiments/traffic_matrix_load/data/traffic_goodput_total_data_sent_vs_runtime.csv" using ($2 / 1e9 * 8):(stringcolumn(1) eq "tcp" ? $3 / 1e9 : 1/0) title "TCP workload" w points ls 1, \
        "../../ns3_experiments/traffic_matrix_load/data/traffic_goodput_total_data_sent_vs_runtime.csv" using ($2 / 1e9 * 8):(stringcolumn(1) eq "udp" ? $3 / 1e9 : 1/0) title "UDP workload" w points ls 2, \
